...
func FundModule(c *C, ctx sdk.Context, k keep.Keeper, name string, amt uint64) {
...
func FundAccount(c *C, ctx sdk.Context, k keep.Keeper, addr sdk.AccAddress, amt uint64) {
	coin, err := common.NewCoin(common.RuneNative, sdk.NewUint(amt*common.One)).Native()
	c.Assert(err, IsNil)
	err = k.Supply().MintCoins(ctx, ModuleName, sdk.NewCoins(coin))
	c.Assert(err, IsNil)
